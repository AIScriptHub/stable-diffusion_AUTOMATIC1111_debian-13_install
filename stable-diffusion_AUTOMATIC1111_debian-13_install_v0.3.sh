...
#
# Usage:
#   bash Installer_-_Debian-13_Stable-Diffusion_AUTOMATIC1111_v0.2_v0.3.sh
#
...
#   3. Redirects TMPDIR/pip cache to disk (important on low-RAM systems,
...
# install. This assumes /tmp itself is disk-backed (not tmpfs) on this
# system - verify with: mount | grep '/tmp'. If /tmp is tmpfs, redirect
# TMPDIR to a location on disk instead (e.g. $HOME/tmp).
# Per-user subdirectories avoid collisions if multiple users run this
# script on the same machine.
setup_tmpdir_on_disk() {
    section "Step 1: Redirecting TMPDIR/pip cache to disk (RAM protection)"

    local tmp_dir="/tmp/sd-webui-tmp-$USER"
    local pip_cache_dir="/tmp/sd-webui-pip-cache-$USER"
    ...
    if ! grep -q "export TMPDIR=" "$HOME/.bashrc" 2>/dev/null; then
        echo "export TMPDIR=$tmp_dir" >> "$HOME/.bashrc"
    fi
...
Environment=PIP_CACHE_DIR=$PIP_CACHE_DIR
